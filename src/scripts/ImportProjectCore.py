...
def importPrj(fname, chunkName):
    ...
    if prjType == "MultiCamera":
        chunk.importCameras(path=image_point_file, format=Metashape.CamerasFormat.CamerasFormatBundler)
    else:
        chunk.importCameras(path=image_point_file, format=Metashape.CamerasFormat.CamerasFormatBundler, image_list=bundler_file_list, load_image_list=True)

    chunk.triangulatePoints(10000)

    ##chunk.importMarkers(markers_file)
    ImportMarkers(markers_file, chunk)
    chunk.importReference(cameras_file, columns='nxyzabco' ,delimiter=' ',\
        group_delimiters=False,skip_rows=0,ignore_labels=False,create_markers=False)
...
def AddSensor(chunk, fname):
    ...
    if (lineList[15].startswith("Master:")):
        ...
        sensor.reference.location = Metashape.Vector([float(lineList[16]), float(lineList[17]), float(lineList[18])])
        sensor.reference.location_accuracy = Metashape.Vector([0.0000001, 0.0000001, 0.0000001])
        sensor.reference.location_enabled = True
        sensor.reference.rotation = Metashape.Vector([float(lineList[19]), float(lineList[20]), float(lineList[21])])
        sensor.reference.rotation_accuracy = Metashape.Vector([0.0000001, 0.0000001, 0.0000001])
...
def ImportMarkers(fname, chunk):
    with open(fname) as fp:
        for line in fp:
            ...
            marker.reference.location = Metashape.Vector((float(items[2]),float(items[3]),float(items[4])))
            nimg = int(items[5])
            for i in range(0,nimg):
                marker.projections[chunk.cameras[int(items[6 + i * 3])]] = \
                    Metashape.Marker.Projection(Metashape.Vector((float(items[7 + i * 3]),float(items[8 + i * 3]))), True)
...
def Optimize(fname, chunk):
    ...
    p1 = bool(lineList[8])
    p2 = bool(lineList[9])
    chunk.optimizeCameras(fit_f=f,fit_cx=cx,fit_cy=cy,fit_b1=b1,fit_b2=b2,\
        fit_k1=k1,fit_k2=k2,fit_k3=k3,fit_k4=False,\
        fit_p1=p1,fit_p2=p2,fit_p3=False,fit_p4=False,\
