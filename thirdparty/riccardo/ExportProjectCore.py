...
def export(filename):
    ...
    try:
        ...
        for pt in points:
            ...
            f.write(
                "{:d}\t{:.8f}\t{:.8f}\t{:.8f}\n".format(
                    pt.track_id, point.x, point.y, point.z
                )
            )
        ...
        for sensor in chunk1.sensors:
            f.write(
                "Sensor:\t{:s}\t{:d}\t{:s}\t{:d}\n".format(
                    sensor.label, sensor.key, str(sensor.type), isMasterSensor(sensor)
                )
            )
            f.write(
                "{:d}\t{:d}\t{:.8f}\t{:.8f}\t{:.8f}\n".format(
            ...
            f.write(
                "{:.8e}\t{:.8e}\t{:.8e}\t{:.8e}\n".format(
                    sensor.calibration.k1,
                    sensor.calibration.k2,
                    sensor.calibration.k3,
                    sensor.calibration.k4,
                )
            )
            f.write(
                "{:.8e}\t{:.8e}\t{:.8e}\t{:.8e}\n".format(
                    sensor.calibration.p1,
                    sensor.calibration.p2,
                    sensor.calibration.p3,
                    sensor.calibration.p4,
                )
            )
            f.write(
            ...
            if not (isMasterSensor(sensor)):
                ...
                f.write(
                    "{:.8f}\t{:.8f}\t{:.8f}\n".format(
                        sensor.location.x, sensor.location.y, sensor.location.z
                    )
                )
                f.write("{:.8f}\t{:.8f}\t{:.8f}\n".format(R.x, R.y, R.z))
                if not (
                    sensor.reference.location is None
                    or sensor.reference.rotation is None
                ):
                    f.write(
                        "{:.8f}\t{:.8f}\t{:.8f}\n".format(
                            sensor.reference.location.x,
                            sensor.reference.location.y,
                            sensor.reference.location.z,
                        )
                    )
                    f.write(
                        "{:.8f}\t{:.8f}\t{:.8f}\n".format(
                            sensor.reference.rotation.x,
                            sensor.reference.rotation.y,
                            sensor.reference.rotation.z,
                        )
                    )
        ...
        for camera in chunk1.cameras:
            if (
                not camera.type == Metashape.Camera.Type.Regular
            ):  # skipping camera track keyframes
                continue
            if not camera.transform:  # skipping NA cameras
                continue
            f.write(
                "Camera:\t{:s}\t{:s}\t{:d}\t{:d}\t".format(
                    camera.label, camera.photo.path, camera.key, camera.sensor.key
                )
            )
            coord = camera.center
            point = crs.project(T.mulp(coord))
        ...
        for camera in chunk1.cameras:
            ...
            for proj in projections[camera]:
                ...
                if not points[point_id].valid:
                    continue
                f.write(
                    "ImagePoint:\t{:s}\t{:d}\t{:d}\t{:.8f}\t{:.8f}\n".format(
                        camera.label, camera.key, track_id, proj.coord.x, proj.coord.y
                    )
                )
        ...
        for marker in chunk1.markers:
            f.write(
                "{:s}\t{:d}\t{:s}\n".format(
                    marker.label, marker.key, str(marker.reference.enabled)
                )
            )
            if marker.reference is None or marker.reference.location is None:
                ...
                f.write(
                    "{:.4f}\t{:.4f}\t{:.4f}\n".format(
                        marker.reference.location.x,
                        marker.reference.location.y,
                        marker.reference.location.z,
                    )
                )
            if marker.reference.accuracy == None:
                ...
                f.write(
                    "{:.4f}\t{:.4f}\t{:.4f}\n".format(
                        marker.reference.accuracy.x,
                        marker.reference.accuracy.y,
                        marker.reference.accuracy.z,
                    )
                )
        ...
        for marker in chunk1.markers:
            for camera in marker.projections.keys():
                ...
                line = "{:s}\t{:d}\t{:d}\t{:.2f}\t{:.2f}\n".format(
                    camera.label, camera.key, marker.key, proj.coord.x, proj.coord.y
                )
                f.write(line)
